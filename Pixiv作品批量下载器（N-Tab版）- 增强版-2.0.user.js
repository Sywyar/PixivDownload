...
`;
    ...
    function updateQueueDisplay() {
        ...
        downloadQueue.forEach((artwork, index) => {
            ...
            if (artwork.status === 'downloading' && artwork.totalImages > 0) {
                // 重要修复：使用实际下载数量而不是当前图片索引来计算进度
                const downloadedCount = artwork.downloadedCount || 0;
                const currentImage = artwork.currentImage || 0;
                
                // 优先使用实际下载数量，如果没有则使用当前图片索引
                const actualProgress = downloadedCount > 0 ? downloadedCount : currentImage;
                const progressPercent = Math.min(Math.round((actualProgress / artwork.totalImages) * 100), 100);

                progressHtml = `
        <div style="margin-top: 3px;">
            <div style="display: flex; justify-content: space-between; font-size: 9px; margin-bottom: 2px;">
                <span>进度: ${actualProgress}/${artwork.totalImages}</span>
    ...
    function startStatusCheck(artworkId) {
        console.warn('startStatusCheck已废弃，请使用SSE连接');
    }

    // 停止状态检查（已废弃，使用SSE替代）
    function stopStatusCheck(artworkId) {
        console.warn('stopStatusCheck已废弃，请使用SSE连接');
    }
    ...
    function updateArtworkStatus(artworkId, status) {
        ...
        if (artwork) {
            ...
            if (status.completed) {
                artwork.status = 'completed';
                artwork.currentImage = status.downloadedCount || status.totalImages;
